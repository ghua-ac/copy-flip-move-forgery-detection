function [im11] = region_m1(image)
tstart = tic;
[im, match_r,loc] = remove_m1(image);
match=match_r;
num=size(match,1);
k=400;%周围距离的平方
l=3;%最小匹配对
...
while num>5
    count=0;
        [r1,r0,im1]=reg1(im,match,loc,r0,k,l,count,image,im1);
        match([r1,r0],:)=[];
        if num==size(match,1)
            num=0;
        else
...
function [nr1,r0,im1] = reg1(im,match,loc,r0,k,l,count,image,im1)
        i=1;
        r=match(i,:);
        num=size(match,1);
        nr1=[];
